  ...
  return (
    (<div style={{ marginTop: 18 }}>
      ...
      <div style={{ maxHeight: 220, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, padding: '4px 2px', marginBottom: 10 }}>
        {messages.length === 0 ? (
          <div className="muted" style={{ fontSize: 12 }}>{tx('No messages yet — keep money decisions next to the numbers.')}</div>
        ) : messages.map(m => {
      ...
      <form onSubmit={send} className="row" style={{ gap: 8 }}>
        <input value={text} onChange={e => setText(e.target.value)} maxLength={2000} placeholder={tx('Message your members…')}
        ...
        <button type="submit" className="btn btn-primary" disabled={!text.trim()}>{tx('Send')}</button>
      </form>
    </div>)
  );
}
