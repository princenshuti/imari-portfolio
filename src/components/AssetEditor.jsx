...
export default function AssetEditor({ asset, onSave, onCancel, showToast }) {
  ...
  // ── Render ───────────────────────────────────────────────
  return (
    (<Modal open onClose={onCancel} maxWidth={720} title={isNew ? tx('Add asset') : tx('Edit asset')}>
      {/* Header */}
      <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
        <div>
          <div className="muted" style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            {isNew ? tx('Add asset') : tx('Edit asset')}
          </div>
          <h2 className="font-serif" style={{ fontSize: 26, marginTop: 2, margin: 0, fontWeight: 400 }}>{a.name || tx('Untitled asset')}</h2>
        </div>
        <button
          type="button" onClick={onCancel} aria-label={tx('Close dialog')}
          className="btn-icon-sm"
        ><span aria-hidden="true">×</span></button>
      </div>
      {/* Asset class — radiogroup, keyboard accessible */}
      <Field label={tx('Asset class')}>
        <div role="radiogroup" aria-label={tx('Asset class')} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
          {CLASSES.map(c => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <Field label={tx('Name')}>
          <Input value={a.name} onChange={v => update('name', v)} placeholder={tx('e.g. Plot in Kabuga')} />
        </Field>
        ...
        <Field label={tx('Purchase price')} hint={tx('in {0}', [a.currency])}>
          <Input value={a.purchasePrice} onChange={v => update('purchasePrice', v)} type="number" placeholder="0" />
        </Field>
        <Field label={tx('Purchase date')}>
          <Input value={a.purchaseDate} onChange={v => update('purchaseDate', v)} type="date" />
        </Field>

        {cls.fields.includes('neighbourhood') && (
          <Field label={tx('Neighbourhood')}>
            <Input value={a.neighbourhood} onChange={v => update('neighbourhood', v)} placeholder={tx('e.g. Kabuga')} />
          </Field>
        )}
        {cls.fields.includes('upi') && (
          <Field label="UPI" hint={tx('Unique Parcel Identifier — from your title deed')}>
            <Input value={a.upi} onChange={v => update('upi', v)} placeholder={tx('e.g. 1/01/01/01/0001')} />
          </Field>
        )}
        {cls.fields.includes('upi') && (
          <Field label={tx('Registered owner (on title)')}>
            <Input value={a.registeredOwner} onChange={v => update('registeredOwner', v)} placeholder={tx('Name as on the NLA e-title')} />
          </Field>
        )}
        {cls.fields.includes('upi') && (
          <Field label={tx('District')}>
            <Input value={a.district} onChange={v => update('district', v)} placeholder={tx('e.g. Kicukiro')} />
          </Field>
        )}
        {cls.fields.includes('upi') && (
          <Field label={tx('Last revalued')} hint={tx('Used to prompt a re-valuation when stale')}>
            <Input value={a.lastRevaluedAt} onChange={v => update('lastRevaluedAt', v)} type="date" />
          </Field>
        )}
        {cls.fields.includes('upi') && (
          <Field label={tx('Owner verification')} hint={tx('Self-check — Imari does no automated NID lookup (privacy)')}>
            <label style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 13, cursor: 'pointer' }}>
              ...
              {tx('Registered owner matches my NID')}
            </label>
          </Field>
        )}
        {cls.fields.includes('propertyCategory') && (
          <Field label={tx('Property category')}>
            <select
              value={a.propertyCategory}
              onChange={e => update('propertyCategory', e.target.value)}
              style={inputStyle}
            >
              <option value="">{tx('— Pick a category —')}</option>
              {PROPERTY_CATEGORIES.map(c => (
                <option key={c.id} value={c.id}>{tx(c.label)}</option>
              ))}
            </select>
          </Field>
        )}
        {cls.fields.includes('sizeM2') && (
          <Field label={tx('Size (m²)')}>
            <Input
              value={a.sizeM2} onChange={v => update('sizeM2', v)}
              type="number" placeholder={tx('e.g. 500')}
            />
          </Field>
        )}
        {cls.fields.includes('model') && (
          <Field label={tx('Model')}>
            <Input value={a.model} onChange={v => update('model', v)} placeholder={tx('e.g. Toyota Rav4 2018')} />
          </Field>
        )}
        {cls.fields.includes('chassis') && (
          <Field label={tx('Chassis / VIN')} hint={tx('From the vehicle logbook')}>
            <Input value={a.chassis} onChange={v => update('chassis', v)} placeholder={tx('e.g. JTMBD33V585012345')} />
          </Field>
        )}
        {cls.fields.includes('vehicleCategory') && (
          <Field label={tx('Vehicle category')} hint={tx('Road levy · Law 013/2025')}>
            <select value={a.vehicleCategory} onChange={e => update('vehicleCategory', e.target.value)} style={inputStyle}>
              {VEHICLE_CATEGORIES.map(c => (
                <option key={c.id} value={c.id}>
                  {tx(c.label)} {tx('— RWF')} {c.levy.toLocaleString()}/ yr
                                    </option>
              ))}
            </select>
          </Field>
        ...
        {cls.fields.includes('monthlyContribution') && (
          <Field label={tx('Monthly contribution')}>
            <Input value={a.monthlyContribution} onChange={v => update('monthlyContribution', v)} type="number" placeholder={tx('e.g. 50000')} />
          </Field>
        )}
        {cls.fields.includes('employerMatch') && (
          <Field label={tx('Employer match / month')}>
            <Input value={a.employerMatch} onChange={v => update('employerMatch', v)} type="number" placeholder={tx('e.g. 50000')} />
          </Field>
        )}
        {cls.fields.includes('currentAge') && (
          <Field label={tx('Your current age')}>
            <Input value={a.currentAge} onChange={v => update('currentAge', v)} type="number" placeholder={tx('e.g. 35')} />
          </Field>
        )}
        {cls.fields.includes('annualSalary') && (
          <Field label={tx('Annual salary (for replacement %)')}>
            <Input value={a.annualSalary} onChange={v => update('annualSalary', v)} type="number" placeholder={tx('e.g. 6000000')} />
          </Field>
        )}
      </div>
      ...
      {suggested != null && (
        <div style={{
          ...
          <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
            ...
            }}>{tx('Use suggestion')}</button>
          </div>
        </div>
      )}
      <Field label={tx('Today\'s value (your number)')} hint={tx('in {0} · leave blank to use suggestion', [a.currency])} top={16}>
        <Input value={a.currentValue} onChange={v => update('currentValue', v)} type="number" placeholder={tx('Leave blank for suggestion')} />
      </Field>
      <Field label={tx('Notes')} top={4}>
          ...
          {a.location.country === 'Rwanda' ? (
            <Field label={tx('Province *')}>
              <select value={a.location.province} onChange={e => updateLoc('province', e.target.value)} style={inputStyle}>
                <option value="">{tx('— Select province')}</option>
                {RWANDA_PROVINCES.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </Field>
          ) : (
            <Field label={tx('Province / State *')}>
              <Input value={a.location.province} onChange={v => updateLoc('province', v)} placeholder={tx('Province or state')} />
            </Field>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {a.location.country === 'Rwanda' ? (
            <Field label={tx('District *')}>
              ...
              >
                <option value="">{tx('— Select district')}</option>
                {districts.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </Field>
          ) : (
            <Field label={tx('District / City *')}>
              <Input value={a.location.district} onChange={v => updateLoc('district', v)} placeholder={tx('District or city')} />
            </Field>
          )}
          <Field label={tx('Sector')} hint="optional">
            <Input value={a.location.sector} onChange={v => updateLoc('sector', v)} placeholder={tx('e.g. Remera')} />
          </Field>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <Field label={tx('Cell')} hint="optional">
            <Input value={a.location.cell} onChange={v => updateLoc('cell', v)} placeholder={tx('e.g. Nyabisindu')} />
          </Field>
          <Field label={tx('Village')} hint="optional">
            <Input value={a.location.village} onChange={v => updateLoc('village', v)} placeholder={tx('e.g. Inganzo')} />
          </Field>
        </div>
      ...
      >
        <Toggle
          ...
          label={tx('This asset generates income')}
        />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
            ...
            <Field label={tx('Frequency')}>
              <select value={a.incomeFrequency} onChange={e => update('incomeFrequency', e.target.value)} style={inputStyle}>
                <option value="monthly">{tx('Per month')}</option>
                <option value="annually">{tx('Per year')}</option>
              </select>
            </Field>
            <Field label={tx('Currency')}>
        ...
        {a.photos.length < 3 ? (
          <>
            ...
            >
              {photoLoading
                ? tx('◌ Compressing…')
                : tx('▣ Upload photo{0} · {1} slot{2} remaining · auto-compressed', [
                3 - a.photos.length > 1 ? 's' : '',
                3 - a.photos.length,
                3 - a.photos.length > 1 ? 's' : ''
              ])}
            </button>
        ...
        {a.documents.length < 5 ? (
          <>
            ...
            >
              {docLoading
                ? tx('◌ Reading…')
                : tx('◧ Attach document{0} · {1} slot{2} remaining · max 2 MB each', [
                5 - a.documents.length > 1 ? 's' : '',
                5 - a.documents.length,
                5 - a.documents.length > 1 ? 's' : ''
              ])}
            </button>
          </>
          ...
          <div style={{ fontSize: 11, color: 'var(--ink-4)', textAlign: 'center', padding: '4px 0' }}>
            {tx('Maximum 5 documents reached.')}
          </div>
        )}
        <div style={{
          fontSize: 11, color: 'var(--ink-4)', lineHeight: 1.55,
      ...
      <div className="row" style={{ gap: 10, marginTop: 24, justifyContent: 'flex-end' }}>
        ...
        >
          {isNew ? tx('Add asset') : tx('Save changes')}
        </button>
        {(!a.name || !a.purchasePrice) && (
          <span id="asset-save-hint" style={{ position: 'absolute', width: 1, height: 1, padding: 0, margin: -1, overflow: 'hidden', clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap', border: 0 }}>
            {tx('Fill in name and purchase price to enable saving')}
          </span>
        )}
      </div>
    </Modal>)
