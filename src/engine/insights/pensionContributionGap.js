...
export default function pensionContributionGap(state, { now = new Date() } = {}) {
  ...
  return makeInsight({
    ...
    type: 'foresight',
    category: 'retirement',
    headline: tx('Your pension is invisible to your net worth'),
    body: tx(
      'You record {0}/month in salary, so at the {1}% statutory RSSB rate roughly {2}/month should be accruing toward your retirement — but the Retirement view has no inputs yet. Two minutes of setup shows whether you\'re on track for 60, and what an Ejo Heza top-up changes.',
      [rwf(salaryMonthly), ratePct, rwf(statutoryMonthly)]
    ),
    costOfAbsence: {
      severity: 'info',
      amount: statutoryMonthly,
      costStatement: tx(
        '~{0}/month of retirement wealth is accruing unseen — untracked savings can\'t be steered.',
        [rwf(statutoryMonthly)]
      ),
      action: { label: tx('Set up Retirement'), to: 'retirement' },
    },
    sourceRefs: ids,
