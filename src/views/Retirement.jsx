...
export default function RetirementView({ state, dispatch }) {
  ...
  if (readiness != null && input.currentAge) {
    if (readiness < 50) {
      coa = { sev: 'critical', headline: tx(
        'Your pension is on track to cover less than half of a comfortable retirement.'
      ),
        body: tx(
          'Replacing {0}% of income at {1} (target: {2}%). Add monthly Ejo Heza to close the gap.',
          [
            result.replacementRatio?.toFixed(0),
            input.retirementAge,
            RETIREMENT_CONFIG.replacementTargetPct
          ]
        ) };
    } else if (readiness < 80) {
      coa = { sev: 'warning', headline: tx('Pension is on track but below the comfort threshold.'),
        body: tx(
          'Replacing {0}% — adding even RWF 10k/month between now and {1} compounds materially.',
          [result.replacementRatio?.toFixed(0), input.retirementAge]
        ) };
    } else {
      coa = { sev: 'info', headline: tx('Pension is on track to a comfortable replacement.'),
        body: tx(
          'On the modeled assumptions, you replace {0}% of working income.',
          [result.replacementRatio?.toFixed(0)]
        ) };
    }
  }
  ...
  return (
    (<div style={{ padding: 28, background: 'var(--bg)', minHeight: 'calc(100vh - 70px)' }}>
      <div className="muted" style={{ fontSize: 13, marginBottom: 18 }}>{tx(
        'A modeled projection grounded in the statutory schedule — never a guarantee.'
      )}</div>
      ...
      <Reveal className="card" style={{ padding: '12px 16px', marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'baseline' }}>
        <div>
          <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 700 }}>{tx('Statutory rate')} {thisYear}</div>
          <div className="num" style={{ fontSize: 18, fontWeight: 700, color: 'var(--brand)' }}>{currentStatRate}%</div>
        </div>
        {nextStep && (
          <div>
            <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 700 }}>{tx('Next step')}</div>
            <div className="num" style={{ fontSize: 14, fontWeight: 600 }}>{nextStep.totalRatePct}{tx('% from')} {nextStep.fromYear}</div>
          </div>
        )}
      ...
      <Reveal delay={0.06} className="card" style={{ padding: '18px 20px', marginBottom: 18 }}>
        <div className="row" style={{ justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14, gap: 10 }}>
          <div className="font-serif" style={{ fontSize: 16 }}>{tx('Your situation')}</div>
          <button onClick={persist} className="btn btn-ghost btn-sm" type="button" aria-label={tx('Save retirement inputs to your profile')}>{tx('Save inputs')}</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14 }}>
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Current age')}</span>
            <input type="number" min="16" max="90" value={currentAge} onChange={e => setCurrentAge(e.target.value)} style={numInput} />
          </label>
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Retirement age')}</span>
          ...
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Annual salary (')}{ccy})</span>
            <input type="number" min="0" value={annualSalary} onChange={e => setAnnualSalary(e.target.value)} style={numInput} />
          </label>
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Contributions to date (')}{ccy})</span>
            <input type="number" min="0" value={contributionsToDate} onChange={e => setContributionsToDate(e.target.value)} style={numInput} />
            <span className="muted" style={{ fontSize: 10 }}>{tx('Total in RSSB + Ejo Heza today')}</span>
          </label>
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Monthly contribution (')}{ccy})</span>
            <input type="number" min="0" value={monthlyContribution} onChange={e => setMonthlyContribution(e.target.value)} style={numInput} />
            <span className="muted" style={{ fontSize: 10 }}>{tx('Your share — RSSB + voluntary')}</span>
          </label>
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Employer match (')}{ccy}/mo)</span>
            <input type="number" min="0" value={employerMatch} onChange={e => setEmployerMatch(e.target.value)} style={numInput} />
          </label>
          <label className="col" style={{ gap: 5 }}>
            <span className="muted" style={{ fontSize: 11 }}>{tx('Real return %/yr')}</span>
            <input type="number" min="0" max="15" step="0.5" value={realReturnPct} onChange={e => setRealReturnPct(e.target.value)} style={numInput} />
            <span className="muted" style={{ fontSize: 10 }}>{tx('Above-inflation pension growth')}</span>
          </label>
        </div>
      </Reveal>
      {/* Results */}
        ...
        <>
          {coa && (
            <div role={coa.sev === 'critical' ? 'alert' : 'status'} style={{
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--ink)' }}>{tx(coa.headline)}</div>
                <div className="muted" style={{ fontSize: 12.5, marginTop: 4, lineHeight: 1.5 }}>{tx(coa.body)}</div>
              </div>
            </div>
          )}
          ...
          <Stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 18 }}>
            <StaggerItem className="card" style={{ padding: '16px 18px' }}>
              <div className="muted" style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tx('Years to retirement')}</div>
              <div className="num" style={{ fontSize: 22, fontWeight: 700, color: 'var(--ink)', marginTop: 4 }}>{result.years}</div>
            </StaggerItem>
            <StaggerItem className="card" style={{ padding: '16px 18px' }}>
              <div className="muted" style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tx('Projected pot at')} {input.retirementAge}</div>
              ...
              <div className="muted" style={{ fontSize: 10, marginTop: 3 }}>{tx('In today\'s purchasing power (real return)')}</div>
            </StaggerItem>
            <StaggerItem className="card" style={{ padding: '16px 18px' }}>
              ...
              <div className="muted" style={{ fontSize: 10, marginTop: 3 }}>{tx('Annuitised over')} {RETIREMENT_CONFIG.yearsInRetirement}years</div>
            </StaggerItem>
            <StaggerItem className="card" style={{ padding: '16px 18px' }}>
              <div className="muted" style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tx('Replacement ratio')}</div>
              ...
              <div className="muted" style={{ fontSize: 10, marginTop: 3 }}>{tx('vs target')} {RETIREMENT_CONFIG.replacementTargetPct}%</div>
            </StaggerItem>
            <StaggerItem className="card" style={{ padding: '16px 18px' }}>
              <div className="muted" style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tx('Readiness score')}</div>
              ...
              <div className="muted" style={{ fontSize: 10, marginTop: 3 }}>{tx('Floored to never overstate')}</div>
            </StaggerItem>
          </Stagger>
        </>
      ) : (
        <div className="card" style={{ padding: '20px 22px', textAlign: 'center' }}>
          <div className="muted" style={{ fontSize: 13 }}>{tx('Enter your current age to see a projection.')}</div>
        </div>
      )}
