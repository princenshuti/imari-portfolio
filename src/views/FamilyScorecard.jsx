...
export default function FamilyScorecard({ portfolioId, role, showToast }) {
  ...
  if (!portfolioId) return <div className="card muted" style={{ padding: 20 }}>{tx('Sign in to use the family scorecard.')}</div>;
  ...
  return (
    (<div className="col" style={{ gap: 16 }}>
      ...
      <div className="row" style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div className="seg-tabs" role="group" aria-label={tx('Scorecard range')}>
        ...
        {tab === 'week' && (
          <div className="row" style={{ gap: 6 }}>
            <button className="btn btn-ghost btn-xs" onClick={() => setWeek(w => addDays(w, -7))} aria-label={tx('Previous week')}>←</button>
            <input type="date" value={week} onChange={e => e.target.value && setWeek(weekStart(e.target.value))} style={{ ...inputStyle, width: 'auto', padding: '6px 10px' }} aria-label={tx('Week of')} />
            <button className="btn btn-ghost btn-xs" onClick={() => setWeek(w => addDays(w, 7))} aria-label={tx('Next week')}>→</button>
            <button className="btn btn-ghost btn-xs" onClick={() => setWeek(weekStart(new Date()))}>{tx('Today')}</button>
          </div>
        )}
        {tab !== 'week' && (
          <div className="row" style={{ gap: 6 }}>
            <button className="btn btn-ghost btn-xs" onClick={() => setYear(y => y - 1)} aria-label={tx('Previous year')}>←</button>
            <span className="num" style={{ fontWeight: 600 }}>{year}</span>
            <button className="btn btn-ghost btn-xs" onClick={() => setYear(y => y + 1)} aria-label={tx('Next year')}>→</button>
          </div>
        )}
      </div>
      {fam.logs === null && !fam.error && <div className="muted" style={{ fontSize: 13 }}>{tx('Loading scorecard…')}</div>}
...
function WeekTab({ fam, week }) {
  ...
  return (<>
    <div className="card" style={{ padding: 18 }}>
      <div className="row" style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, alignItems: 'baseline' }}>
        <div>
          ...
          <div className="muted" style={{ fontSize: 12 }}>{tx('Streak:')} {streak(logs, week)}week{streak(logs, week) === 1 ? '' : 's'} {tx('at 23+ ·')} {canEdit ? tx('ticks save instantly') : tx('view only')}</div>
        </div>
        <div className="row" style={{ gap: 12, alignItems: 'baseline' }}>
          ...
          <span className="pill" style={{ background: `${r.color}22`, color: r.color }}>{tx(r.label)}</span>
        </div>
      </div>
        ...
        return (
          (<div key={s.id} className="card" style={{ padding: 16, borderTop: `3px solid ${s.color}` }}>
            <div className="row" style={{ justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ fontWeight: 600, fontSize: 14 }}>{tx(s.title)}</div>
    ...
    <div className="card" style={{ padding: 16 }}>
      <label htmlFor="week-reflection" style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink-2)' }}>{tx('Weekly reflection')}</label>
      <div className="muted" style={{ fontSize: 11, marginBottom: 6 }}>{tx('Top wins · what slipped · one key adjustment')}</div>
...
function MonthTab({ logs, year }) {
  ...
  return (<>
    <div className="row muted" style={{ gap: 16, fontSize: 12, flexWrap: 'wrap' }}>
      <span>{tx('Best month:')} <b>{best ? best.name : '—'}</b></span>
      <span>{tx('Year average:')} <b className="num">{logged.length ? `${Math.round(logged.reduce((a, b) => a + b.avg, 0) / logged.length)}/${MAX_SCORE}` : '—'}</b></span>
      <span>{tx('Weeks logged:')} <b className="num">{months.reduce((a, b) => a + b.logged, 0)}</b></span>
      <span>{tx('Excellent weeks:')} <b className="num">{months.reduce((a, b) => a + b.excellent, 0)}</b></span>
    </div>
    ...
    {sel != null && (
      <div className="card" style={{ padding: 16 }}>
        <div className="font-serif" style={{ fontSize: 18, marginBottom: 8 }}>{tx(MONTHS[sel])} {year}</div>
        {months[sel].weeks.map(w => {
          ...
          return (
            (<div key={w} className="row" style={{ gap: 12, padding: '8px 0', borderBottom: '1px solid var(--line-soft)', fontSize: 13 }}>
              ...
              <span className="muted" style={{ minWidth: 80, fontSize: 12 }}>{s == null ? tx('Not logged') : tx(r.label)}</span>
            </div>)
          );
        })}
      </div>
...
function YearTab({ logs, year }) {
  ...
  return (<>
    <div className="row muted" style={{ gap: 16, fontSize: 12, flexWrap: 'wrap' }}>
      <span>{tx('Year average:')} <b className="num">{y.avg == null ? '—' : `${y.avg}/${MAX_SCORE}`}</b></span>
      <span>{tx('Strongest area:')} <b>{y.best ? tx(y.best.title) : '—'}</b></span>
      <span>{tx('Weeks logged:')} <b className="num">{y.weeks}</b></span>
    </div>
    <div className="card" style={{ padding: 16 }}>
      <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 10 }}>{tx('Monthly heatmap')}</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(56px, 1fr))', gap: 8 }}>
        {heat.map(h => {
          const r = rating(h.avg || 0);
          return (
            (<div key={h.n} title={h.avg == null ? tx('{0}: no data', [h.n]) : `${h.n}: ${h.avg}/${MAX_SCORE}`}
    ...
    <div className="dash-kpi-grid">
      {y.sections.map(s => (
        <div key={s.id} className="card" style={{ padding: 14, borderTop: `3px solid ${s.color}` }}>
          <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 600 }}>{tx(s.title)}</div>
          ...
          <div className="muted" style={{ fontSize: 11 }}>{tx('Average per week')}</div>
        </div>
      ))}
    </div>
  </>);
...
function ImportCard({ portfolioId, onDone, showToast }) {
  ...
  async function onFile(e) {
    ...
    e.target.value = '';
    if (!file) return;
    if (file.size > MAX_IMPORT_BYTES) { setMsg(tx('File too large (max 1 MB).')); return; }
    setBusy(true); setMsg('');
    try {
      const raw = JSON.parse(await file.text());
      const mapped = mapLegacyExport(raw);
      const n = await importLegacy(portfolioId, mapped);
      setMsg(tx('Imported {0} habit ticks and {1} notes.', [n.logs, n.notes]));
      showToast?.(tx('Family history imported'), 'success');
      onDone?.();
    } catch (err) {
      setMsg(tx('Import failed: {0}', [err.message || 'invalid file']));
    ...
    (<div className="card" style={{ padding: 16 }}>
      <div className="row" style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontWeight: 600, fontSize: 13 }}>{tx('Import from the 2026 standalone dashboard')}</div>
        ...
        <button className="btn btn-sm" disabled={busy} onClick={() => fileRef.current?.click()}>{busy ? tx('Importing…') : tx('Choose file')}</button>
      </div>
    </div>)
  );
}
