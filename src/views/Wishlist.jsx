...
export default function Wishlist({ state, dispatch, portfolioId, role, showToast }) {
  ...
  async function toGoal(w) {
    ...
    showToast?.(tx('Goal created from this wish'), 'success');
  }
  ...
  return (
    (<div className="col" style={{ gap: 16 }}>
      ...
      <div className="dash-kpi-grid">
        <KPI label={tx('Open wishes')} value={open.length} sub={tx('{0} in total', [money(total, cur)])} />
        <KPI label={tx('Affordable now')} value={open.filter(w => w.a.status === 'now').length} sub={tx('Without breaking the 3-month buffer')} accent="var(--up)" />
        <KPI label={tx('Investable today')} value={money(investable, cur)} sub={tx('Liquid cash minus 3 months of spend')} />
        <KPI label={tx('Savings pace')} value={`${money(pace, cur)}/mo`} sub={pace > 0 ? tx('Income minus spending (6-month avg)') : tx('Not saving yet — wishes cannot get closer')} accent={pace > 0 ? 'var(--brand)' : 'var(--down)'} />
      </div>
      <div className="card" style={{ padding: 18 }}>
        <PageHead title={tx('Wish list')} sub={tx('Ranked by priority. The verdict updates as cash and spending change.')}
          action={canEdit && <button className="btn btn-primary btn-sm" onClick={() => setEditing('new')}>{tx('+ Wish')}</button>} />
        {items === null && !error && <Empty>{tx('Loading…')}</Empty>}
        {items && open.length === 0 && <Empty>{tx(
          'Nothing wished for yet. Add the vehicle, the trip, the furniture — with a cost — and Imari tells you when it fits.'
        )}</Empty>}
          ...
          {(showDone ? wishes : open).map(w => {
            ...
            return (
              (<div key={w.id} className="row" style={{ gap: 12, padding: '9px 0', borderTop: '1px solid var(--line-soft)', alignItems: 'center', flexWrap: 'wrap' }}>
                <input type="checkbox" checked={w.status === 'done'} disabled={!canEdit} onChange={e => setStatus(w.id, e.target.checked ? 'done' : 'open')} style={{ accentColor: 'var(--up)' }} aria-label={tx('Bought: {0}', [w.title])} />
                <button className="btn-unstyled" onClick={() => canEdit && setEditing(w)} style={{ flex: 1, minWidth: 180, textAlign: 'left', cursor: canEdit ? 'pointer' : 'default' }}>
                  <div style={{ fontSize: 13, fontWeight: 600, textDecoration: w.status === 'done' ? 'line-through' : 'none' }}>{tx(w.title)}</div>
                  <div className="muted" style={{ fontSize: 11 }}>{PRIO_LABEL[w.data?.priority] || tx('Should have')} · {FOR_LABEL[w.data?.for] || tx('Family')}{w.due_date ? tx(' · by {0}', [w.due_date]) : ''}</div>
                </button>
                <span className="num" style={{ fontSize: 13, fontWeight: 600 }}>{money(w.amount, cur)}</span>
                {w.status === 'open' && (
                  <span className={`pill ${v.cls}`} style={{ fontSize: 10 }} title={w.a.gap ? tx('Short by {0}', [money(w.a.gap, cur)]) : ''}>
                    {tx(v.label)}{w.a.status !== 'now' && w.a.monthsToAfford ? tx(' · ~{0} mo', [w.a.monthsToAfford]) : ''}
                  </span>
                )}
                {w.data?.link && <a className="btn btn-ghost btn-xs" href={w.data.link} target="_blank" rel="noopener noreferrer nofollow">{tx('Link ↗')}</a>}
                {canEdit && w.status === 'open' && <button className="btn btn-ghost btn-xs" onClick={() => toGoal(w)}>{w.data?.goal_id ? tx('View goal →') : tx('Make it a goal')}</button>}
              </div>)
            );
          })}
        </div>
        {done.length > 0 && <button className="btn btn-ghost btn-xs" style={{ marginTop: 8 }} onClick={() => setShowDone(v => !v)}>{showDone ? tx('Hide bought') : tx('Show {0} bought', [done.length])}</button>}
      </div>
