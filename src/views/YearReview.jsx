    ...
    return (
      (<div style={{ padding: 28, background: 'var(--bg)', minHeight: 'calc(100vh - 70px)' }}>
        <div className="card" style={{ padding: 60, textAlign: 'center' }}>
          <div className="font-serif" style={{ fontSize: 22, marginBottom: 8 }}>{tx('Your year is still being written')}</div>
  ...
  return (
    (<div style={{ padding: 28, background: 'var(--bg)', minHeight: 'calc(100vh - 70px)' }}>
      ...
      <Reveal className="card" style={{ padding: '24px 26px', marginBottom: 14 }}>
        <div className="muted" style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{tx('Net worth this year')}</div>
      ...
      <Stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 14 }}>
        {[
          { label: tx('Highest point'), value: review.high.netWorth, sub: shortDate(review.high.date), color: 'var(--up)' },
          { label: tx('Lowest point'), value: review.low.netWorth, sub: shortDate(review.low.date), color: 'var(--down)' },
          review.bestMonth && { label: tx('Best month'), value: review.bestMonth.change, sub: review.bestMonth.label, color: 'var(--up)', signed: true },
          review.worstMonth && { label: tx('Toughest month'), value: review.worstMonth.change, sub: review.worstMonth.label, color: review.worstMonth.change < 0 ? 'var(--down)' : 'var(--ink-3)', signed: true },
          ...
          <StaggerItem key={c.label} className="card" style={{ padding: '14px 16px' }}>
            ...
            <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>{tx(c.sub)}</div>
          </StaggerItem>
        ))}
      </Stagger>
      {/* Month-by-month change bars */}
      <Reveal className="card" style={{ padding: '18px 22px', marginBottom: 14 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 14 }}>{tx('Month by month')}</div>
        <div
          role="img"
          aria-label={tx('Month-by-month net worth change: {0}', [
            review.months.map(m => `${m.label} ${m.change >= 0 ? 'up' : 'down'} ${fmtBase(Math.abs(m.change), ccy, { compact: true })}`).join(', ')
          ])}
          ...
          {review.months.map(m => {
            ...
            return (
              (<div key={m.ym} title={tip} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 4, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--ink-4)' }}>{tx(m.label)}</div>
              </div>)
            );
          })}
        </div>
      ...
      <Reveal className="card" style={{ padding: '18px 22px' }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>{tx('The whole year')}</div>
        <AreaChart
