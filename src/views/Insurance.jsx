  ...
  return (
    (<div className="col" style={{ gap: 16 }}>
      ...
      <div className="dash-kpi-grid">
        <KPI label={tx('Policies')} value={policies.length} sub={tx('{0} renewing within 60 days', [renewing])} accent={renewing ? 'var(--gold-ink)' : 'var(--brand)'} />
        <KPI label={tx('Premiums / year')} value={money(annual, cur)} sub={tx('All policies, annualised')} />
        <KPI label={tx('Total cover')} value={money(cover, cur)} sub={tx('Sum of cover amounts')} />
        <KPI label={tx('Uninsured assets')} value={gaps.length} sub={gaps.length ? gaps.map(a => a.name || a.kind).join(', ') : tx('Every vehicle and house is linked to a policy')} accent={gaps.length ? 'var(--down)' : 'var(--up)'} />
      </div>
      {gaps.length > 0 && (
        <div className="card" style={{ padding: 14, borderLeft: '3px solid var(--down)' }}>
          <div style={{ fontWeight: 600, fontSize: 13 }}>{tx('Cost of absence')}</div>
          <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>{tx(
            'A single accident or fire on an uninsured asset is a total loss out of family cash. Link or add a policy:'
          )}</div>
          <div className="row" style={{ gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
            {gaps.map(a => <button key={a.id} className="btn btn-sm" disabled={!canEdit} onClick={() => setEditing({ preset: { title: `${a.kind === 'vehicle' ? 'Motor' : 'Home'} · ${a.name || ''}`.trim(), type: a.kind === 'vehicle' ? 'motor' : 'home', asset_id: a.id } })}>{tx('+ Policy for')} {a.name || a.kind}</button>)}
          </div>
        </div>
      )}
      <div className="card" style={{ padding: 18 }}>
        <PageHead title={tx('Policies')} sub={tx('Renewal dates feed the family calendar.')}
          action={canEdit && <button className="btn btn-primary btn-sm" onClick={() => setEditing({})}>{tx('+ Policy')}</button>} />
        {items === null && !error && <Empty>{tx('Loading…')}</Empty>}
        {items && policies.length === 0 && <Empty>{tx(
          'No policies yet. Start with health (RSSB / mutuelle or private), then motor, home and life.'
        )}</Empty>}
          ...
          {policies.map(p => (
            <div key={p.id} className="row" style={{ gap: 12, padding: '9px 0', borderTop: '1px solid var(--line-soft)', alignItems: 'center', flexWrap: 'wrap' }}>
              <button className="btn-unstyled" onClick={() => canEdit && setEditing({ initial: p })} style={{ flex: 1, minWidth: 200, textAlign: 'left', cursor: canEdit ? 'pointer' : 'default' }}>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{tx(p.title)}</div>
                <div className="muted" style={{ fontSize: 11 }}>
                  {TYPE[p.data?.type] || tx('Other')}{p.data?.insurer ? ` · ${p.data.insurer}` : ''}{p.data?.asset_id && assetName(p.data.asset_id) ? tx(' · covers {0}', [assetName(p.data.asset_id)]) : ''}
                  {p.data?.cover ? tx(' · cover {0}', [money(p.data.cover, cur)]) : ''}
                </div>
              </button>
              ...
              {canEdit && p.amount && !p.data?.cashflow_id && <button className="btn btn-ghost btn-xs" onClick={() => pushPremium(p)} title={tx('Record the premium as a recurring expense in Cash Flow')}>{tx('→ Cash Flow')}</button>}
              {p.data?.cashflow_id && <span className="pill pill-soft" style={{ fontSize: 10 }}>{tx('in Cash Flow')}</span>}
            </div>
          ))}
        </div>
      </div>
