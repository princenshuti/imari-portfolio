...
export default function Documents({ portfolioId, role, showToast }) {
  ...
  async function onFile(e) {
    ...
    try {
      ...
      showToast?.(tx('File stored in the vault'), 'success');
    } catch (err) { showToast?.(err.message || tx('Upload failed'), 'error'); }
  ...
  async function openFile(doc) {
    ...
    catch (err) { showToast?.(err.message || tx('Could not open file'), 'error'); }
  }
  ...
  return (
    (<div className="col" style={{ gap: 16 }}>
      ...
      <div className="dash-kpi-grid">
        <KPI label={tx('Documents')} value={docs.length} sub={tx('{0} with a stored file', [withFile])} />
        <KPI label={tx('Expiring ≤ 90 days')} value={expiring.length} sub={expiring.length ? expiring.map(d => d.title).slice(0, 3).join(', ') : tx('Nothing expiring soon')} accent={expiring.length ? 'var(--gold-ink)' : 'var(--brand)'} />
        <KPI label={tx('Storage')} value="Private" sub={tx('Files open via 60-second links only')} />
        <KPI label={tx('Max file')} value={`${DOC_MAX_BYTES / 1048576} MB`} sub={tx('PDF, JPG, PNG, WebP')} />
      </div>
      <div className="card" style={{ padding: 18 }}>
        <PageHead title={tx('Vault')} sub={tx(
          'IDs, passports, land titles, contracts, school and medical records — metadata here, the scan attached.'
        )}
          action={canEdit && <button className="btn btn-primary btn-sm" onClick={() => setEditing('new')}>{tx('+ Document')}</button>} />
        {items === null && !error && <Empty>{tx('Loading…')}</Empty>}
        {items && docs.length === 0 && <Empty>{tx(
          'Nothing here yet. Add each family member\'s ID and passport first, with the expiry date, so renewals reach the calendar.'
        )}</Empty>}
          ...
          {docs.map(d => (
            <div key={d.id} className="row" style={{ gap: 12, padding: '9px 0', borderTop: '1px solid var(--line-soft)', alignItems: 'center', flexWrap: 'wrap' }}>
              <button className="btn-unstyled" onClick={() => canEdit && setEditing(d)} style={{ flex: 1, minWidth: 200, textAlign: 'left', cursor: canEdit ? 'pointer' : 'default' }}>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{tx(d.title)}</div>
                <div className="muted" style={{ fontSize: 11 }}>
                  {TYPE[d.data?.type] || tx('Other')}{d.data?.holder ? ` · ${d.data.holder}` : ''}{d.data?.ref_hint ? ` · ···${d.data.ref_hint}` : ''}{d.due_date ? tx(' · expires {0}', [d.due_date]) : ''}
                </div>
              </button>
              <Due days={daysUntil(d.due_date, now)} />
              {d.data?.file?.path ? (
                <>
                  <button className="btn btn-sm" onClick={() => openFile(d)} title={d.data.file.name}>{tx('Open ·')} {kb(d.data.file.size || 0)}</button>
                  {canEdit && <button className="btn btn-ghost btn-xs" disabled={busyId === d.id} onClick={() => pickFile(d)}>{busyId === d.id ? tx('Uploading…') : tx('Replace')}</button>}
                </>
              ) : canEdit && (
                <button className="btn btn-ghost btn-sm" disabled={busyId === d.id} onClick={() => pickFile(d)}>{busyId === d.id ? tx('Uploading…') : tx('Attach file')}</button>
              )}
            </div>
          ))}
        </div>
